...
// ------------------------------------------------------------
// Objective = sum distances + fairness penalty
// ------------------------------------------------------------
...
std::vector<double> calc_my_metric(const Instance& I, double a) {
    ...
    double max_dist = 0.0;
    for (double v : solo) max_dist = std::max(max_dist, v);

    int max_dem_int = 0;
    for (int c : I.demands) max_dem_int = std::max(max_dem_int, c);

    if (max_dist == 0.0) max_dist = 1.0;
    if (max_dem_int == 0) max_dem_int = 1;
