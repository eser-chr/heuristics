...
int main(int argc, char **argv)
{
    ...
    std::cout << " I am starting with beam search tuning" << std::endl;
    ...
    for (auto N : Ns)
    {
        ...
        std::cout << " I am starting instances with " << N_str << " requests" << std::endl;
        ...
        {
            Instance I(instance);
            auto sol = BS::beam_search(I, alpha, beam_width);
            auto f_sol = utils::objective(I, sol);
            res.push_back(RES{instance, N, alpha, beam_width, f_sol});
        }
    }

    write_csv_results(base_output / "beam_tuning_validation.csv", res);
}
