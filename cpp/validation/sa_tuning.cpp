    ...
    for (auto N : Ns)
    {
        ...
        for (auto const &instance : instance_paths)
        {
            ...
            for (auto iters : Ls)
            {
                ...
                auto sol = SA::simulated_annealing(I, dr_sol, neighs, T0, Tmin, alpha, StepFunction::first_improvement, stopping_criterion);
                auto f_sol = utils::objective(I, sol);
                res.push_back(RES{N, f_sol, T0, iters, alpha, instance});
            }
            std::cout << "\n\n";
        }
    }
